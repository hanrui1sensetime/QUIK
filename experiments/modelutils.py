...
def get_fp_features_num(module: torch.nn.Linear, args, model=None):
    ...
    if getattr(args, "fp_relative", False) and model is not None:
        fp_features_num = int(
            module.in_features / model.config.hidden_size) * fp_features_num
...
def find_layers(module,
                layers=[
                    torch.nn.Linear, quant_sim.ActQuantWrapper,
...
def get_opt(model):
    ...
    model = transformers.OPTForCausalLM.from_pretrained(model,
                                                        torch_dtype='auto')
...
def get_llama(model_name, seqlen, hf_token):
    ...
    model = transformers.LlamaForCausalLM.from_pretrained(
        model_name, torch_dtype='auto', use_auth_token=hf_token, config=config)
