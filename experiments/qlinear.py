...
class MixedQLinear(torch.nn.Module):
        ...
        if shared_input.qint_x is None:
            ...
            # elif self.fp_features_num > 0:
                ...
                shared_input.qint_x, shared_input.meta, shared_input.fp_x = quik.asymmetric.quantize(
                    x, self.int_indices, self.fp_indices, self.bits)
            # else:
    ...
    def quantize_kv(self, x):
        ...
        qint_x, meta, _ = quik.asymmetric.quantize(x, cache_int_indices,
                                                   cache_fp_indices,
                                                   self.kvcache_bit)
    ...
    def dequantize_kv(self, x):
        ...
        reduced_w = torch.ones((1, x.shape[-1]),
                               dtype=torch.float16,
                               requires_grad=False).to(x.device)
        output = quik.asymmetric.dequantize(x, self.kvcache_meta,
                                            weights_scales, reduced_w,
